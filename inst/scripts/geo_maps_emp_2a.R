...
packages <- c("sf", "fst", "tidyverse", "dplyr", "data.table", "tmap", "leaflet")
...
tenpercent_dt<- read.fst("//esapov/esapov/MOZ/GEO/Population/tenpercent_cleancensus.fst",
                         ...
                         as.data.table=TRUE)
bairro_dt<- read.fst("//esapov/esapov/MOZ/GEO/Population/Daylan/bairro.fst")
...
##Cell phone towers per sqkm in each neighborhood.
  ...
  tmap::tm_shape(emp_dt) +
    tmap::tm_polygons() +
    tm_basemap(server = 'https://tiles.stadiamaps.com/tiles/alidade_smooth_dark/{z}/{x}/{y}{r}.png',
               group ="Stadia.AlidadeSmoothDark", alpha = NA, tms = FALSE) +
    tm_tiles('https://tiles.stadiamaps.com/tiles/alidade_smooth_dark/{z}/{x}/{y}{r}.png',
             group ="Stadia.AlidadeSmoothDark", alpha = 1, zindex = NA, tms = FALSE) +
    tm_bubbles(size = "Tower_per_sqkm", col = "red")
